...
For publish workflows with SBOM, provenance, and signing: → See
...
The CLI shows a live terminal UI during execution with progress, timing, and
cache status for every operation.

### OpenTelemetry traces

Dagger emits OTel spans for every operation. Export to Jaeger, Honeycomb, or any
compatible backend:
...
Dagger Cloud provides a web UI for visualizing traces, drilling into logs, and
monitoring cache performance across local and CI runs:
