...
Use these resources when the skill's patterns don't cover a specific method,
option struct, or advanced feature. Listed in priority order.
...
Option structs follow the pattern `<Type><Method>Opts`: `ContainerBuildOpts`,
`ContainerWithExecOpts`, `ContainerWithDirectoryOpts`, `ContainerAsServiceOpts`,
`ContainerWithFileOpts`, etc.
...
Search for common tasks: golang, python, node, docker-compose, trivy, helm,
golangci-lint, argocd, terraform, and many more.
...
Modules on Daggerverse have documentation pages showing available functions,
arguments, and usage examples. Always pin to a version tag.
...
Check the SDK version in `go.mod` (e.g. `dagger.io/dagger v0.19.x`) and search
accordingly — some methods are added or changed between versions.
