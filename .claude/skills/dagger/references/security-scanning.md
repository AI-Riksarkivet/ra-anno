...
Call:
`dagger call generate-sbom --format spdx-json export --path ./sbom.spdx.json`
...
Key gotcha: Don't set `OTEL_EXPORTER_OTLP_ENDPOINT` as a container env var when
using `WithServiceBinding` — Dagger rewrites env vars containing service
