...
Complete guide to caching strategies for Transformers.js models across different
environments.
...
Transformers.js models can be large (from a few MB to several GB), so caching is
critical for performance. The caching strategy differs based on the environment:
...
Caching is **automatic** and enabled by default. Models are cached after the
first download.
...
In browser environments, Transformers.js uses the
[Cache API](https://developer.mozilla.org/en-US/docs/Web/API/Cache) to store
models:
...
**Tip:** Monitor storage usage with the
...
if ("storage" in navigator && "estimate" in navigator.storage) {
  ...
  console.log(
    `Available: ${
      ((estimate.quota - estimate.usage) / 1024 / 1024).toFixed(2)
    } MB`,
  );
}
```
...
**Note:** The statement "Models are cached automatically in
`~/.cache/huggingface/`" from performance tips is specific to Hugging Face's
Python tooling convention. In Transformers.js for Node.js, the default is
