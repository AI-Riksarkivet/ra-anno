...
Comprehensive reference for Storybook in SvelteKit 2 / Svelte 5 projects using
the Svelte CSF v5 addon.

**Framework:** `@storybook/sveltekit` **Story format:**
`@storybook/addon-svelte-csf` v5 (Svelte CSF) **Docs:**
https://storybook.js.org/docs/svelte/get-started
...
This scaffolds `.storybook/` config and installs dependencies. If addons are
missing:
...
> Importing `../src/app.css` is essential for Tailwind classes and shadcn-svelte
> CSS variables to work in Storybook.
...
Storybook Svelte CSF v5 has several ways to define templates. Choose based on
your needs:
...
Content placed directly in `<Story>` is static — it won't react to Controls
changes:
...
3. Sharing template across some stories? → Define root snippet, pass via
   `template` prop
...
Storybook provides experimental mocking for SvelteKit modules via
...
Stories are referenced by their export name (derived from `name` prop or
explicit `exportName`).
...
When a component uses Svelte 5 snippets (like `children`, `header`, `footer`),
they appear as `Snippet` types. For Controls, cast them to strings:
...
Make sure you're using `{#snippet template(args)}` (not
`{#snippet children(args)}`). Static content inside `<Story>` won't react to
Controls changes.
