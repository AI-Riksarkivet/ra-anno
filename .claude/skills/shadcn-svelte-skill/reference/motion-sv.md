...
A port of the Motion library (formerly Framer Motion) specifically for Svelte 5.
It aligns closely with the **motion-v** API structure rather than the React
version.
...
ALWAYS pass styles as an object via `style={{ key: value }}`, never as a string.
This is required to bind `MotionValue`s correctly without triggering re-renders.
...
For better developer experience and type safety, define variants using the
`Variants` type.
...
- **Gestures:** `whileHover`, `whilePress` (preferred over `whileTap`),
  `whileDrag`, `whileFocus`.
- **Drag:** `drag` (boolean | "x" | "y"), `dragConstraints`, `dragElastic`,
  `dragMomentum`.
- **Events:** `onAnimationStart`, `onAnimationComplete`, `onUpdate`.
- **Gesture Events:** `onHoverStart`, `onHoverEnd`, `onPress`, `onPressStart`,
  `onPressEnd`.
...
Enables exit animations. _Modes:_ `"sync"` (default), `"wait"`, `"popLayout"`.
...
Svelte lacks `getSnapshotBeforeUpdate`. You **MUST** use `createLayoutMotion`
...
- **API Alignment:** Follow `motion-v` prop naming (e.g., `inViewOptions`,
  `whilePress`).
- **No Svelte transitions:** Do NOT use Svelte's `transition:fn` directives
  alongside motion-sv.
- **Styles:** Always use the object syntax `style={{ ... }}`.
- **Layout animations:** Always use `createLayoutMotion` — don't try `layout`
  prop directly.
- **Tailwind classes:** Regular `class` prop works fine on `motion.*` elements —
  use it for non-animated styles, and `style={{ }}` for animated values.
