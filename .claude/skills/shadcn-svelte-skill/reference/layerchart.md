...
Composable Svelte chart components for data visualization, integrated with
shadcn-svelte.

**Package:** `layerchart@next` (v2 pre-release, Svelte 5) **Docs:**
https://next.layerchart.com (NOT layerchart.com — that's Svelte 4)
**shadcn-svelte integration:**
https://www.shadcn-svelte.com/docs/components/chart
...
These wrap the lower-level primitives and provide props for common config (data,
...
- **Marks**: `Area`, `Bars`, `Spline`, `Points`, `Pie`, `Labels` — data-driven
  visuals
- **Common**: `Axis`, `Grid`, `Legend`, `Frame`, `Rule` — chart furniture
- **Interactions**: `Tooltip`, `TooltipContext`, `Highlight`, `BrushContext`,
  `TransformContext`
...
shadcn-svelte does **not** wrap LayerChart — you use LayerChart components
directly and only bring in the shadcn `Chart` wrapper for consistent tooltip
styling:
...
The `Chart.Container` provides consistent sizing and the `Chart.Tooltip` matches
your shadcn theme.
...
The `--chart-1` through `--chart-5` variables defined in `app.css` map to
Tailwind classes via `@theme inline`:
...
> **Note:** For Storybook stories that import LayerChart, ensure
> `layerchart@next` resolves correctly — verify `nodeModulesDir: "auto"` in
> `deno.json`.
