...
All examples use the recommended default Markdown extensions configuration. If
extensions have been customized, some features may require explicit enabling.
...
**Required extensions:** `admonition`, `pymdownx.details`,
...
**Important:** Inline admonitions must appear _before_ the content they sit
beside.
...
Per-block: `{ .yaml .copy }`, `{ .yaml .no-copy }`, `{ .yaml .select }`,
`{ .yaml .no-select }`
...
**Required extensions:** `pymdownx.superfences`, `pymdownx.tabbed` (with
...
```html
<div class="grid cards" markdown>
  - :material-clock-fast:{ .lg .middle } __Set up in 5 minutes__ --- Install
  Zensical and get up and running in minutes. [:octicons-arrow-right-24: Getting
  started](#) - :fontawesome-brands-markdown:{ .lg .middle } __It's just
  Markdown__ --- Focus on your content and generate a responsive site.
...
<div class="grid" markdown>
  :fontawesome-brands-html5: __HTML__ { .card } :fontawesome-brands-js:
  __JavaScript__ { .card } > A regular blockquote (not a card)
</div>
```
...
````html
<div class="grid" markdown>
  === "Tab A" * Item 1 * Item 2 ``` python print("Hello") ```
</div>
````
...
```markdown
:fontawesome-regular-face-laugh-wink: :lucide-check: :material-material-design:
:octicons-mark-github-16: :simple-simpleicons:
```
...
```markdown
[Subscribe](#){ .md-button } [Subscribe](#){ .md-button .md-button--primary }
...
```markdown
![Title](image.png){ align=left } ![Title](image.png){ align=right }
```
...
```markdown
![Title](image.png){ width="300" } /// caption Caption text ///
```
...
```markdown
![Light](image-light.png#only-light) ![Dark](image-dark.png#only-dark)
```
...
: Definition paragraph. Can contain multiple paragraphs indented by 4 spaces.
```
...
**Required extensions:** `pymdownx.caret`, `pymdownx.keys`, `pymdownx.mark`,
`pymdownx.tilde`

```markdown
==Highlighted text== ^^Underlined text^^ ~~Strikethrough text~~

H~2~O (subscript) A^T^A (superscript)
...
```markdown
This paragraph is excluded from search. { data-search-exclude }
```
