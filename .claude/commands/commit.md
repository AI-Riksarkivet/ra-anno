Help create a Conventional Commit following the specification
(https://www.conventionalcommits.org/en/v1.0.0/).

First, run `git status --short` and `git diff --cached --stat` and
`git diff --stat` to show current changes.

Then follow these rules for the commit format: **<type>[optional scope]:
<description>**
...
1. **Type is REQUIRED** (feat, fix, docs, style, refactor, perf, test, build,
   ci, chore, revert)
...
6. **Footer is OPTIONAL** - uses format: token: value or BREAKING CHANGE:
   description
